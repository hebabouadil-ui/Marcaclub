...
export default function CustomerAuthModal({ onSuccess, onClose }: Props) {
  ...
  // returnTo = current page so the user lands back here after Google auth
      ...
      <div className="bg-white w-full max-w-sm shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        {tab === 'forgot' && forgotSent ? (
          <div className="px-6 py-8 text-center space-y-4">
            ...
            <button onClick={() => switchTab('login')} className="text-xs text-brand-gray underline hover:text-brand-black transition-colors">
          ...
          <form onSubmit={handleSubmit} className="px-6 py-6 space-y-4">
            ...
            {/* Google — shared component, same redirect flow everywhere */}
            {tab !== 'forgot' && (
              <>
                <GoogleButton
                  returnTo={returnTo}
                  label="Continuer avec Google"
                  className="w-full flex items-center justify-center gap-3 border border-gray-200 py-2.5 text-sm font-medium text-brand-black hover:bg-gray-50 transition-colors"
                />
            ...
            {tab === 'register' && (
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} required placeholder="Sophie M."
